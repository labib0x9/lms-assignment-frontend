...
export default function AdminDashboard({ user }) {
  ...
  const roleLabel = (user?.role?.name || user?.role?.type || "Administrator").toUpperCase();
  ...
  return (
    <div className="min-h-screen bg-[#faf9f6] text-[#0a192f] flex flex-col selection:bg-amber-200 selection:text-amber-900">
      <DashboardNav
        user={user}
        roleLabel={roleLabel}
        roleBadgeStyle="bg-purple-100 text-purple-900 border border-purple-300/60"
      />
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-6 py-8 sm:py-12 space-y-8">
        ...
        <div className="bg-white border border-slate-200/90 rounded-3xl p-6 sm:p-10 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div className="space-y-2 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-50 border border-purple-200 text-xs font-semibold text-purple-900">
            ...
            <p className="text-sm sm:text-base text-slate-600 leading-relaxed">
              Global control center to oversee all courses, lessons, instructors, and student enrollments.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200/90 shadow-xs space-y-1">
            ...
            <p className="text-3xl font-bold text-purple-900">Unrestricted</p>
            <p className="text-xs text-slate-500">Full CRUD access on all courses</p>
          </div>
        ...
        <div className="space-y-5">
          <div>
            <h2 className="text-xl font-bold text-[#0a192f]">
              All Platform Courses ({courses.length})
            </h2>
            <p className="text-xs text-slate-500">
              Select any course to manage its lessons, edit details, or create assessments.
            </p>
          </div>
          ...
          {!loadingCourses && courses.length === 0 && (
            <div className="bg-white border border-slate-200/90 rounded-2xl p-12 text-center space-y-3 shadow-xs">
              ...
              <p className="text-xs text-slate-500 max-w-sm mx-auto">
                Create the first course on the platform to start publishing learning materials.
              </p>
              <button
                onClick={() => setIsModalOpen(true)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {courses.map((course) => {
                ...
                return (
                  <div
                    key={docId}
                    className="bg-white border border-slate-200/90 rounded-2xl p-6 shadow-xs flex flex-col justify-between space-y-4 transition-all hover:border-slate-300 hover:shadow-sm"
                  >
